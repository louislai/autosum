...
def binary() : 
  blognames,words,data=readfile('data.txt')

  '''blog cluster'''
  clust=hcluster(data)
  printclust(clust,labels=blognames)
  drawdendrogram(clust,blognames,jpeg="clust.jpg")

  '''word cluster'''
  rdata=rotatematrix(data)
  wordclust=hcluster(rdata)
  drawdendrogram(wordclust,labels=words,jpeg='other_clust.jpg')

def kMeanClustering(n=3):
  blognames,words,data=readfile('data.txt')

  '''blog cluster'''
  kclust=kcluster(data,k=n)
  print kclust
...
def cluster_articles(url_list):
	...
	for idx, data in enumerate(data_list):
		article_name_list.append('Article ' + str(idx+1))
